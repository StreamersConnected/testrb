...
class AdminPlugin(Plugin):
    ...
    @Plugin.command('block', '<user:user|snowflake> [reason:string]', level=CommandLevels.ADMIN)
    def block(self, event, user, reason=None):
        """
        Blocks the user from viewing the current channel.
        """
        member = event.guild.get_member(user)
        if member:
            self.can_act_on(event, member.id)
        #    event.channel.create_overwrite(member, allow=0)
            self.client.api.channels_pernissions_modify(event.channel.id, member.id, allow=None, deny=Permissions.READ_MESSAGES, typ='member', reason=reason)
            if event.config.confirm_actions:
                event.msg.reply(u':ok_hand: denied `READ_MESSAGES` from **{}**'.format(member.user))
        else:
            raise CommandFail('invalid user')

    @Plugin.command('unblock', '<user:user|snowflake> [reason:string]', level=CommandLevels.ADMIN)
    def unblock(self, event, user, reason=None):
        """
        Edits a specfic user's overwrites to neutral.
        """
        member = event.guild.get_member(user)
        if member:
            self.can_act_on(event, member.id)
            self.client.api.channels_permissions_delete(event.channel.id, member.id, reason=reason)
        #    event.channel.create_overwrite(member, deny=0)
            if event.config.confirm_actions:
                event.msg.reply(u':ok_hand: neutralised permission overwrites for **{}**'.format(member.user))
        else:
            raise CommandFail('invalid user')
