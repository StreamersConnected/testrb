...
class UtilitiesPlugin(Plugin):
    ...
    @Plugin.command('clear', group='status', level=CommandLevels.ADMIN)
    def command_clear(self, event):
        self.client.update_presence(status=Status.ONLINE)
        event.msg.reply('Ok, status cleared')

    @Plugin.command('plug', '<stream:str> <msg:str...>', group='status', level=CommandLevels.ADMIN)
    ...
    @Plugin.command('hex', '<val:int>', group='util', global_=True)
    def cmd_hex(self, event, val):
        if val > 9223372036854775807:
            return event.msg.reply(u':warning: number too large')
        event.msg.reply(str(hex(val)))

    @Plugin.command('int', '<val:str>', group='util', global_=True)
    def cmd_int(self, event, val):
        try:
            num = int(val, 16)
            if num > 9223372036854775807:
                return event.msg.reply(u':warning: number too large')
            event.msg.reply(str(num))
        except ValueError:
            event.msg.reply('cannot convert `{}` to `int`'.format(val))
