...
BOT_INFO = '''
Based on Rowboat ( Copyright (c) 2015-2017 Andrei Zbikowski ), a moderation and utilitarian bot built for large Discord servers.
'''
...
class CorePlugin(Plugin):
    ...
    def send_control_message(self):
        embed = MessageEmbed()
        embed.set_footer(text='Rezruel\'s Power Bot {}'.format(
    ...
    def command_about(self, event):
        embed = MessageEmbed()
        embed.set_author(name='Rowboat', icon_url=self.client.state.me.avatar_url, url='http://192.168.10.107/')
    ...
    def command_uptime(self, event):
        event.msg.reply("Rezruel's Power Bot was started {} ago".format(
